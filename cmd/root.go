...
func init() {
	...
	RootCmd.PersistentFlags().IntVarP(&frontendPort, "frontend-service-port", "f", 8084, "Port for frontend service")
...
func onInitialize() {
	...
	if frontendPort != 8084 {
		logger.Info("changing frontend service port", zap.Int("old", 8084), zap.Int("new", frontendPort))
	}
