...
import (
	...
	"terazo_parcel_service/pkg/pool"
	"terazo_parcel_service/services/config"
	"terazo_parcel_service/services/customer"
	"terazo_parcel_service/services/driver"
	"terazo_parcel_service/services/route"
)
...
func (eta *bestETA) Get(ctx context.Context, customerID string) (*Response, error) {
	...
	if resp.Driver == "" {
		...
		eta.logger.Bg().Info("no routes found", zap.String("customer", customerID))
	}
...
func (eta *bestETA) getRoutes(ctx context.Context, customer *customer.Customer, drivers []driver.Driver) []routeResult {
	...
	for _, dd := range drivers {
		...
		eta.pool.Execute(func() {
			...
			eta.logger.Bg().Info("finding routes for driver", zap.String("customer location", customer.Location))
			routesLock.Lock()
			...
			eta.logger.Bg().Info(" found routes for driver", zap.String("driver", driver.DriverID))
			routesLock.Unlock()
			wg.Done()
		})
	}
